...
for (i in 1:length(L)) {
  ...
  # Minimal illuminance
  # Cleveland plot
  dotchart(L[[i]]$TS,pch=16,col='blue',xlab=paste0("Temperature du sol sur ",names(L[i])," jours "))
  # Histogram
  hist(L[[i]]$TS,col='blue',xlab= paste0("Temperature du sol sur ",names(L[i])," jours "),main="")
  # Quantile-Quantile plot
  qqnorm(L[[i]]$TS,pch=16,col='blue',xlab='')
  qqline(L[[i]]$TS,col='red')
}
...
for (i in 1:length(L)) {
  ...
  #température du sol
  plot(L[[i]]$NDVI_moyen_BV ~L[[i]]$TS,pch=16,col='blue',xlab=paste0('Température moyenne du     sol sur ', names(L[i])," jours"),ylab='NDVI moyen du BV sur 15 jours')
}
```
...
for (i in 1:length(L)) {
  ...
  #température du sol
  plot(L[[i]][month(L[[i]]$Debut) >= 4 & month(L[[i]]$Fin) <=9,"NDVI_moyen_BV"] ~L[[i]][month(L[[i]]$Debut) >= 4 & month(L[[i]]$Fin) <=9,"TS"],pch=16,col='blue',xlab=paste0('Température moyenne du sol sur ', names(L[i])," jours"),ylab='NDVI moyen du BV sur 15 jours')
}
```
MOUAI 